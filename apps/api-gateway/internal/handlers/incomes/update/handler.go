...
// Handler maneja las peticiones de actualización de ingresos
type Handler struct {
	service            incomes.IncomeService
	gamificationHelper *services.GamificationHelper
}

// NewHandler crea una nueva instancia del handler de actualización de ingresos
...
func (h *Handler) UpdateIncome(c *gin.Context) {
	...
	response, err := h.service.UpdateIncome(c.Request.Context(), userID, id, &request)
	if err != nil {
	...
	httpUtil.SendSuccess(c, http.StatusOK, response, "Income updated successfully")
}
