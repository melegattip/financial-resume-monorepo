// Package domain defines the core business entities and their behavior for budgets.
package domain
...
const (
	BudgetStatusOnTrack  BudgetStatus = "on_track" // Under alert threshold
	BudgetStatusWarning  BudgetStatus = "warning"  // At or above alert threshold but under 100%
...
// SetAmount sets the budget amount.
func (b *BudgetBuilder) SetAmount(amount float64) *BudgetBuilder {
	b.budget.Amount = amount
	return b
}

// SetPeriod sets the budget period and calculates period start/end dates.
...
// SetAlertAt sets the alert threshold (0.0-1.0).
func (b *BudgetBuilder) SetAlertAt(threshold float64) *BudgetBuilder {
	b.budget.AlertAt = threshold
	return b
}

// setPeriodDates calculates and sets PeriodStart and PeriodEnd based on the current time.
...
// Build validates and returns the constructed Budget.
...
// Validate validates the budget business rules.
...
// UpdateSpentAmount updates the spent amount and recalculates the status.
...
// updateStatus recalculates the budget status based on the spent percentage.
...
// GetSpentPercentage returns the fraction of budget spent (0.0 to 1.0+).
...
// GetRemainingAmount returns the remaining budget amount (never negative).
...
// IsAlertTriggered returns true when the spent percentage meets or exceeds the alert threshold
// but the budget is not yet exceeded.
func (b *Budget) IsAlertTriggered() bool {
	return b.GetSpentPercentage() >= b.AlertAt && b.Status != BudgetStatusExceeded
}

// IsInCurrentPeriod returns true if the current time falls within the budget period.
func (b *Budget) IsInCurrentPeriod() bool {
	now := time.Now()
	return now.After(b.PeriodStart) && now.Before(b.PeriodEnd)
}

// ResetForNewPeriod resets spent amount and recalculates period dates for the current time.
...
// setPeriodDatesFromCurrent updates PeriodStart/PeriodEnd based on the current time.
...
// NewBudgetID generates a new prefixed budget ID.
